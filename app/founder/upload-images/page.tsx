...
export default function FounderUploadImagesPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (!session) {
        router.replace("/login?redirect=/founder/upload-images");
        return;
      }
      if (session.user.id !== FOUNDER_USER_ID) {
        setAuthed(false);
        setLoading(false);
        return;
      }
  ...
  async function handleUpload(itemId: string, file: File) {
    ...
    try {
      ...
      const path = `menu-items/${FRANKFURTER_WIRTSHAUS_ID}/${itemId}.jpg`;
  ...
  if (loading) {
    return (
      <div className="flex min-h-dvh items-center justify-center text-sm" style={{ background: "#06040e", color: "rgba(242,242,242,0.5)" }}>
        Lädt …
      </div>
    );
  }
  if (!authed) {
    return (
      <div className="flex min-h-dvh items-center justify-center px-6 text-center text-sm" style={{ background: "#06040e", color: "rgba(242,242,242,0.65)" }}>
        Nur für den Founder freigegeben.
      </div>
    );
  }
