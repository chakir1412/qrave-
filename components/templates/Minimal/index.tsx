...
export default function MinimalTemplate(props: SpeisekarteProps) {
  ...
  const bgTheme = resolveBackground("minimal", backgroundMode as BackgroundMode | null, customBgColor, customTextColor);
...
/** Sichtbare Diät-Filter-Pills: nur die mit mindestens einem matching-Tag. */
  ...
  return (
    <div className="speisekarte-template" style={{ background: bgTheme.bg, color: bgTheme.text, minHeight: "100dvh" }}>
      ...
      `}</style>
      {!consentGiven && <ConsentBanner locale={locale} restaurantId={restaurantId} onConsent={() => setConsentGiven(true)} />}
      <div className="min-template" style={{ maxWidth: 430, margin: "0 auto", width: "100%", paddingBottom: 90, background: COL.white, minHeight: "100dvh" }}>
        ...
        <div className="min-scrollbar-hide tab-bar-fade" style={{ display: "flex", gap: 8, padding: "12px 16px", overflowX: "auto", borderBottom: `1px solid ${COL.border}`, background: COL.white, position: "sticky", top: 57, zIndex: 9 }}>
          {mainTabs.map((tab) => {
            ...
            return (
              <button key={tab.key} type="button" onClick={(e) => { setPickedMainTab(tab.key); setFilter("all"); if (tab.key !== LUNCH_TAB_KEY) trackCategoryTabSelect(categoryTabLabel(tab.key)); (e.currentTarget as HTMLButtonElement).scrollIntoView({ behavior: 'smooth', inline: 'center', block: 'nearest' }); }} style={catPill(active)}>
                    ...
                    {items.map((item, i) => {
                      ...
                      return (
                        <button key={item.id} type="button" ref={(el) => onItemCardRef?.(item, el as HTMLElement | null)} onClick={() => pushModal(item)} className="min-item" style={{ display: "flex", gap: 12, alignItems: "flex-start", padding: 16, borderBottom: `1px solid ${COL.border}`, cursor: "pointer", transition: "background 0.15s", width: "100%", background: COL.white, border: "none", borderBottomWidth: 1, borderBottomStyle: "solid", borderBottomColor: COL.border, textAlign: "left", opacity: soldOut ? 0.5 : 1, color: "inherit" }}>
                          ...
                          <div style={{ width: 72, minWidth: 72, height: 72, borderRadius: 10, background: "linear-gradient(135deg, #f0f0f0, #e4e4e4)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 30, flexShrink: 0, overflow: "hidden" }}>
                            {hasImg ? (
        ...
        <footer style={{ padding: "16px 20px", textAlign: "center", background: COL.white, borderTop: `1px solid ${COL.border}` }}>
          <p style={{ fontSize: 10, color: COL.muted, margin: 0, letterSpacing: "0.06em" }}>
            ...
            {" · "}
            <PrivacySettingsLink theme="default" locale={locale} restaurantId={restaurantId} color={COL.muted} />
          </p>
        </footer>
      </div>
